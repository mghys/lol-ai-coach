...
class OCRService:
    def __init__(self):
        self.reader = None
        self.champions = {c.lower().replace("'", "").replace(" ", ""): c for c in CHAMPION_LIST}
        self._init_reader()
    ...
    def extract_game_data(self, image_path):
        ...
        try:
            results["ally_team"] = self._extract_team(img, 0, int(w * 0.45), is_ally=True)
            results["enemy_team"] = self._extract_team(img, int(w * 0.55), w, is_ally=False)
        except:
            pass
    ...
    def _detect_victory_defeat(self, img, h, w):
        ...
        text = " ".join([str(r) for r in result]).lower() if result else ""
